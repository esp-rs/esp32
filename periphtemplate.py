...
def parse_periph(svdfile, pname):
    ...
    for rtag in ptag.iter('register'):
        ...
        rsize = int(rtag.find('size').text)
        roffset = int(rtag.find('addressOffset').text, 16)
        rreset = rtag.find('resetValue').text
        ...
        registers[roffset] = {"name": rname, "size": rsize, "reset": rreset, "fields": fields}
    return registers
...
def make_template(pname, registers):
    ...
    for roffset in sorted(registers.keys()):
        ...
        rname = register['name']
        rreset = register['reset']
        rsize = register['size']
        out.append("    {}:".format(rname))
        out.append("      size: {}".format(rsize))
        out.append("      addressOffset: {}".format(roffset))
        out.append("      resetValue: {}".format(rreset))
        out.append("      fields:")
        for foffset in reversed(sorted(register['fields'].keys())):
            ...
            out.append("          bitOffset: {}".format(foffset))
            out.append("          bitWidth: {}".format(fwidth))
        out.append("")
    return "\n".join(out)
